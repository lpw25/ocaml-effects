...
let ctx_matcher p =
  ...
  match p.pat_desc with
  | Tpat_construct (_, cstr, _, omegas) ->
      begin match cstr.cstr_tag with
      | Cstr_extension _ ->
          let nargs = List.length omegas in
          (fun q rem -> match q.pat_desc with
          | Tpat_construct (_, cstr', _, args)
...
let rec exc_inside p = match p.pat_desc with
  | Tpat_construct (_,{cstr_tag=Cstr_extension _},_,_) -> true
...
and split_constr cls args def k =
  ...
  | Tpat_construct (_,{cstr_tag=Cstr_extension _},_,_) ->
