...
and pattern_desc =
  ...
  | Tpat_construct of
      Longident.t loc * constructor_description * bool * pattern list
...
let iter_pattern_desc f = function
  ...
  | Tpat_construct(_, cstr, _, patl) -> List.iter f patl
...
let map_pattern_desc f d =
  ...
  | Tpat_construct (lid, c, t, pats) ->
      Tpat_construct (lid, c, t, List.map f pats)
  | Tpat_array pats ->
