...
let rec compat p q =
  ...
  | Tpat_construct (_, c1, _, ps1), Tpat_construct (_, c2, _, ps2) ->
...
let rec pretty_val ppf v =
  ...
  | Tpat_construct (_, cstr, _, [w]) ->
      fprintf ppf "@[<2>%s@ %a@]" cstr.cstr_name pretty_arg w
  | Tpat_construct (_, cstr, _, vs) ->
...
| Tpat_construct(_, cstr, _, args) -> args
...
let rec normalize_pat q = match q.pat_desc with
  ...
  | Tpat_construct  (lid, c, t, args) ->
      make_pat
        (Tpat_construct (lid, c, t, omega_list args))
...
| {pat_desc = Tpat_record (omegas,closed)} ->
    ...
    make_pat
      ...
      q.pat_type q.pat_env::
    rest
| {pat_desc = Tpat_construct (lid, c, t, omegas)} ->
    let args,rest = read_args omegas r in
    make_pat
      (Tpat_construct (lid, c, t, args))
      q.pat_type q.pat_env::
...
| ({pat_desc = Tpat_construct(_,_,true,_)},_) :: _ -> true
| ({pat_desc = Tpat_construct(_,c,false,_);pat_type=typ},_) :: _ ->
...
let pat_of_constr ex_pat cstr =
 {ex_pat with pat_desc =
  Tpat_construct (mknoloc (Longident.Lident "?pat_of_constr?"),
                  cstr,false,omegas cstr.cstr_arity)}
...
| ({pat_desc = Tpat_construct (lid,
      ({cstr_tag=Cstr_extension _} as c),_,_)},_) :: _ ->
    let c = {c with cstr_name = "*extension*"} in
      make_pat (Tpat_construct(lid, c, false, [])) Ctype.none Env.empty
...
let rec le_pat p q =
  ...
  | Tpat_constant(c1), Tpat_constant(c2) -> const_compare c1 c2 = 0
  | Tpat_construct(_,c1,_,ps), Tpat_construct(_,c2,_,qs) ->
...
| Tpat_construct(lid, c1, t1, ps1), Tpat_construct(_, c2 ,t2, ps2)
      when  c1.cstr_tag = c2.cstr_tag  ->
        let t = t1 || t2 in
        let rs = lubs ps1 ps2 in
        make_pat (Tpat_construct(lid, c1, t, rs))
    ...
    let rec loop pat =
      ...
      | Tpat_tuple lst ->
          ...
          List.map
            ...
            results
      | Tpat_construct (cstr_lid, cstr, total, lst) ->
...
let contains_extension pat =
  ...
  let rec loop = function
      {pat_desc=Tpat_construct(_, {cstr_name="*extension*"}, _, _)} ->
        r := true
...
| Tpat_construct(_, {cstr_tag=(Cstr_constant _|Cstr_block _)}, _, ps) ->
    let path =  get_type_path p.pat_type p.pat_env in
    List.fold_left
...
| Tpat_construct (_, {cstr_tag=Cstr_extension _}, _, ps)->
