...
(* Auxillary type for extension kinds with untranslated defaults. *)
type unfinised_kind =
  | Default of
      constructor_arguments * core_type option * Parsetree.effect_default
  ...
  let args, ret_type, ukind =
    ...
    | Peff_decl(sargs, sret, sedef) ->
        ...
        let ukind =
          ...
          | Some sedef -> Default(targs, tret_type, sedef)
        in
          args, ret_type, ukind
    | Peff_rebind lid ->
        let args, ret_type, kind =
